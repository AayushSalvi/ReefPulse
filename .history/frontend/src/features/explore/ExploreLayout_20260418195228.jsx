...
import { searchCaBeachesDatastoreSql, normalizeCaBeachRecord } from "../../api/caGovBeaches";
import {
...
function ExploreLayout() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const out = await searchCaBeachesDatastoreSql(q, { limit: 30, signal: ac.signal });
      ...
      <div className="ex-grid">
        ...
        <aside className="ex-sidebar" aria-label="Search and filters">
          ...
          <ExploreSidebarSearchInput
            ...
            typeaheadLoading={!hideCaLive && caLoading && query.trim().length >= 1}
          ...
          {hideCaLive && (
            <p className="ex-sidebar-hint">
              California database search is off while activity or demo-only filters are on.
            </p>
          )}
          <ul className="ex-beach-list">
            {!hideCaLive && caError && (
              <li className="ex-beach-list-msg ex-beach-list-msg--error" role="status">
                {caError}
              </li>
            )}
            ...
            {!hideCaLive &&
              ...
              !caError &&
              caResults.length === 0 && (
                <li className="ex-beach-list-msg">No matches in the state beach list for that text.</li>
              )}
            {!hideCaLive &&
              caResults.map((beach) => (
                <li key={beach.id}>
