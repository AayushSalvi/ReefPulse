  ...
  return (
    <div className="ex-map-card ex-google-map ex-google-map--missing-key">
      <div className="ex-google-map-missing-inner">
        <p className="ex-google-map-missing-title">Map search needs an API key</p>
        <p className="ex-google-map-missing-text">
          Add <code className="ex-google-map-code">VITE_GOOGLE_MAPS_API_KEY</code> to <code className="ex-google-map-code">frontend/.env</code> and
          restart the dev server. Enable <strong>Maps JavaScript API</strong> and <strong>Places API</strong>, billing, and allow this origin on the key
          (e.g. <code className="ex-google-map-code">http://localhost:5173/*</code>).
        </p>
      </div>
    </div>
  );
...
export default function ExploreGoogleMapPanel() {
  ...
  if (loadError) {
    return (
      <div className="ex-map-card ex-google-map ex-google-map--error" role="alert">
        <p className="ex-google-map-missing-title">Could not load Google Maps</p>
        <p className="ex-google-map-missing-text">
          Typical fixes: enable billing on the Google Cloud project, turn on <strong>Maps JavaScript API</strong> and <strong>Places API</strong>, and add
          this site’s origin under API key restrictions (HTTP referrers). Check the browser console for the exact error code.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="ex-map-card ex-google-map">
      <p className="ex-google-map-lede">
        Use the left <strong>Search</strong>: pick a suggestion (California or demo) to zoom the map, or type a place and press <strong>Enter</strong> to
        geocode and zoom. You can still click the map to move the pin.
      </p>
      ...
      <div className="ex-google-map-coords" aria-live="polite">
        <div className="ex-google-map-coord">
          ...
          <span className="ex-google-map-coord-value">{formatCoord(markerPosition.lat)}</span>
        </div>
        <div className="ex-google-map-coord">
          <span className="ex-google-map-coord-label">Longitude</span>
          <span className="ex-google-map-coord-value">{formatCoord(markerPosition.lng)}</span>
        </div>
        <p className="ex-google-map-hint">
          {hasCoords ? "Lat/lng are also in the page URL (shareable)." : "Choose a place from the sidebar search to set coordinates."}
        </p>
      </div>
    </div>
  );
