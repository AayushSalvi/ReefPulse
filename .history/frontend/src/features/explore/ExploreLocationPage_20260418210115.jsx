 ...
 *   Past — narrative + timeline slider (illustrative until live data)
 *   Forecast — compare blocks + 7-day chips (illustrative until live data)
 *   Community — local posts with photo placeholders + tips
 */
import { useEffect, useMemo, useState } from "react";
import { Link, Navigate, useParams, useSearchParams } from "react-router-dom";
import {
  fetchCaBeachByExploreId,
...
import { exploreLocationFromCaNormalized } from "./exploreCaBeachViewModel";
...
function ExploreLocationPage() {
  ...
  const mockLocation = findLocation(locationId);
  const [caLoad, setCaLoad] = useState({
    loading: Boolean(locationId?.startsWith("ca-beach-") && !mockLocation),
    location: null,
    error: null
  });
  ...
  useEffect(() => {
    if (mockLocation) {
      setCaLoad({ loading: false, location: null, error: null });
      return undefined;
    }
    if (!locationId?.startsWith("ca-beach-")) {
      setCaLoad({ loading: false, location: null, error: null });
      return undefined;
    }
    const ac = new AbortController();
    setCaLoad({ loading: true, location: null, error: null });
    (async () => {
      const out = await fetchCaBeachByExploreId(locationId, { signal: ac.signal });
      if (ac.signal.aborted) return;
      if (!out.success || !out.record) {
        setCaLoad({
          loading: false,
          location: null,
          error: out.error || "Beach not found in California database"
        });
        return;
      }
      const norm = normalizeCaBeachRecord(out.record);
      const vm = exploreLocationFromCaNormalized(norm);
      setCaLoad({
        loading: false,
        location: vm,
        error: vm ? null : "Could not build location view"
      });
    })();
    return () => ac.abort();
  }, [locationId, mockLocation]);

  const location = mockLocation || caLoad.location;
  const tab = sp.get("tab") || "overview";
  const planning = tab === "past" ? "past" : tab === "forecast" ? "future" : "present";
  const [pastDays, setPastDays] = useState(7);

  const speciesProb = useMemo(() => {
    const prev = location?.speciesPreview;
  ...
  if (!mockLocation && caLoad.loading) {
    return (
      <div className="ex-location">
        <p style={{ margin: "1rem 0", color: "#64748b" }}>Loading California beach record…</p>
      </div>
    );
  }

  if (!mockLocation && locationId?.startsWith("ca-beach-") && caLoad.error) {
    return (
      <div className="ex-location">
        <p style={{ margin: "1rem 0", color: "#b45309" }}>{caLoad.error}</p>
  ...
  const preview = location.speciesPreview || [];
    ...
    <div className="ex-location">
      <div className="ex-loc-body">
        ...
        <div className="ex-loc-center">
          {location.dataCaGov && (
            <div className="ex-ca-banner" role="status">
              <strong>California monitored beach</strong> (data.ca.gov). Waves, safety index, and forecast tabs are
              placeholders until live sensors are connected — follow your county environmental health office for
              advisories
              {location.caAgencyName ? ` (${location.caAgencyName})` : ""}.
            </div>
          )}
              ...
              {tab === "overview" && (
                <div className="ex-panel">
                  ...
                  <p>
                    ...
                    {preview.length
                      ? preview.join(", ")
                      : "— Use Marine life and community for species; this listing does not include a species list."}
                  </p>
                </div>
              )}
              ...
              {tab === "past" && (
                <div className="ex-panel">
                  ...
                  <p>
                    ...
                    {preview.length
                      ? `${preview.join(", ")} (illustrative copy — connect iNaturalist for this site).`
                      : "No curated species list for this state record — check Marine life and community."}
                  </p>
              ...
              {tab === "forecast" && (
                <div className="ex-panel">
                  ...
                  <p style={{ marginBottom: "0.75rem" }}>
                    Calm water, medium visibility, low algal risk — higher chance of {preview[0] || "nearshore fish"}{" "}
                    and {preview[1] || "kelp-zone species"} (model confidence ~70%, illustrative).
                  </p>
                  ...
                  {speciesProb.map((row) => (
                    <p key={row.name} className="ex-spec-prob">
                      {row.name}: ~{row.pct}% in favorable vis ({planning === "future" ? "next 48h" : planning === "past" ? "same calendar week prior" : "today"}) — illustrative.
                    </p>
                  ))}
                  {speciesProb.length === 0 && (
                    <p className="ex-spec-prob" style={{ color: "#64748b" }}>
                      No species probability chart for this state listing.
                    </p>
                  )}
                </div>
              )}
        ...
        <aside className="ex-rail" aria-label="Community and alerts">
          <div className="ex-rail-card">
            ...
            {railSightings.length === 0 ? (
              <p style={{ margin: "0 0 0.6rem", fontSize: "0.82rem", color: "#64748b" }}>
                No ReefPulse posts tagged to this beach yet.
              </p>
            ) : (
