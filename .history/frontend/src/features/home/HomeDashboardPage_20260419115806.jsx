  ...
  useEffect(() => {
    ...
    const separateAndClamp = () => {
      ...
      HERO_FISH_SCHOOL.forEach((cfg, i) => {
        Object.assign(
          posRef.current[i],
          clampInSwim(posRef.current[i].x, posRef.current[i].y, cfg.size),
        );
      });
    };
...
function HomeDashboardPage() {
  ...
  useEffect(() => {
    const urls = [
      ...beachThumbs.map((url, i) => ({
        section: "safestBeach",
        key: `beach-${i}`,
        url,
      })),
      ...species.map((s) => ({
        section: "trendingMarineLife",
        key: s.name,
        url: s.img,
      })),
    ];
    ...
    agentDebugLog(
      "H7",
      "HomeDashboardPage.jsx:image-check:start",
      "Starting home image verification",
      {
        total: urls.length,
      },
    );
    ...
    Promise.all(
      ...
      agentDebugLog(
        "H7",
        "HomeDashboardPage.jsx:image-check:result",
        "Completed home image verification",
        {
        ...
        <div className="home-section-head home-marine-head">
          <div>
            ...
            <p className="home-marine-lede">
              From the safest shoreline picks above to what is moving offshore —
              a quick read on species our community and sensors are watching
              this week.
            </p>
          </div>
